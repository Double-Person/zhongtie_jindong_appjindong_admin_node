<template>
  <a-modal :maskClosable="false"
    ...
    <a-spin :spinning="confirmLoading">
      <preWorkSpeechContentList ref="preWorkSpeechContentList" :editStatus="editStatus" @change="handleChange"/>
    </a-spin>
  </a-modal>
</template>
...
<script>
  ...
  import PreWorkSpeechContentList from './PreWorkSpeechContentList'
  export default {
    name: "PreWorkSpeechContentListModal",
    components: {
      PreWorkSpeechContentList,
    },
    props: {
        editStatus: {
          type: Boolean,
    ...
    data () {
      return {
        ...
        url: {
          add: "/bp/PreWorkSpeechContentList/add",
          edit: "",
        },
        myConfig: {
          // 编辑器不自动被内容撑高
      ...
      edit (record) {
        ...
        this.$nextTick(() => {
          ...
          if(Object.keys(record).length == 0) {
            this.$refs.preWorkSpeechContentList.onClearSelected()
          } else {
            this.$refs.preWorkSpeechContentList.onSetSelected(record.selectionRows)
          }
        });
